/**
 * @file my_player.cpp
 * @brief Реализация синхронизации состояния и выбора хода игроком.
 *
 * Файл поддерживает кеш SearchState, подтягивает новый ход из
 * ядра или полностью перезагружает позицию при рассинхронизации.
 * Затем генерирует кандидатов, сразу играет очевидный выигрыш или блокировку,
 * а для остальных случаев запускает NegamaxSearcher.
 *
 */
